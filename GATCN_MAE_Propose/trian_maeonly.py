...
def train(config=None):
    with wandb.init(name='pose_maeonly'):
        ...
        cls_head=LSTMClassifier(mae_hidden_feature_size,LSTM_model_parameters).to(device)
        ...
        #评估模型
        def evaluate(mae_model, cls_head, dataloader, device):
            mae_model.eval()
            cls_head.eval()
        ...
        # 在训练完后评估模型
        val_accuracy, precision, recall, f1, mcc, auc_roc,preds,labels = evaluate(mae_model, cls_head, val_loader, device)
        wandb.log({
