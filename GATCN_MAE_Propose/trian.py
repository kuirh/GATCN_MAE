...
sweep_configuration = {
    ...
    "parameters": {
        #data parameters
        "mae_learning_rate": {"values": [0.001]},
        "atgcn_learning_rate": {"values": [0.0001]},
        "cls_learning_rate": {"values": [0.0001]},
        "gat":{"values": ['gat','gcn']},
        "st_attention":{"values": [True,False]},
}
}
...
def train(config=None):
    with wandb.init(name='pose'):
        ...
        # Model parameters
        mae_hidden_feature_size = 64  # Size of the feature dimension
        num_encoder_layers = 4  # Number of layers in the encoder
        num_decoder_layers = 4  # Number of layers in the decoder

        # Instantiate the model

        mae_model = MaskedAutoencoder(combined_features=19*4,hidden_feature_size=mae_hidden_feature_size, num_encoder_layers=num_encoder_layers, num_decoder_layers=num_decoder_layers).to(device)
        ...
        cls_head=LSTMClassifier(tempcov2_model_parameters['hidden_channel']* 19+mae_hidden_feature_size,LSTM_model_parameters).to(device)
        ...
        optimizer = optim.Adam([
            {'params': mae_model.parameters(), 'lr': wandb.config.mae_learning_rate},
        ...
        #评估模型
        def evaluate(mae_model, atgcn_model, cls_head, dataloader, device):
            mae_model.eval()
            atgcn_model.eval()
            cls_head.eval()
            ...
            with torch.no_grad():
                for data, labels in dataloader:
                    ...
                    _, encoded, _ = mae_model(data)
                    atgcn_feature = atgcn_model(data, edge_index)

                    combined_features = torch.cat([atgcn_feature, encoded], dim=2)
                    outputs = cls_head(combined_features)
        ...
        for epoch in range(num_epochs):
            ...
            for data, labels in train_loader:
                ...
                reconstructed, encoded, mask = mae_model(data)
                atgcn_feature=atgcn_moael(data, edge_index)
                outputs = cls_head(torch.cat([atgcn_feature,encoded],dim=2))

                if epoch < 15:
                    reconstruction_loss = reconstruction_criterion(reconstructed[~mask], data[~mask])
                    ...
                    reconstruction_loss_weight = 1 - (epoch - 15) / 25
                    reconstruction_loss = reconstruction_criterion(reconstructed[~mask], data[~mask])* reconstruction_loss_weight
                    wandb.log({"reconstruction_loss": reconstruction_loss.item()})
                    classification_loss = classification_criterion(outputs, labels)
                    wandb.log({"classification_loss": classification_loss.item()})
                    loss = reconstruction_loss + classification_loss
        ...
        # 在训练完后评估模型
        val_accuracy, precision, recall, f1, mcc, auc_roc,preds,labels = evaluate(mae_model, atgcn_moael, cls_head, val_loader, device)
        wandb.log({
